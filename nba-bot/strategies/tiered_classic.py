"""
Strategy 2b: Tiered Classic
Original tiered exit logic (1.75x/3x/5x multipliers, 80c late game lock).
Entry logic matches the original tiered strategy with 40c max entry.
Kept as a separate strategy for A/B comparison against Tiered V2.
"""
import logging
...
from core.config import (
    ...
    TIER_CLASSIC_CAPITAL_RECOVERY_MULT, TIER_CLASSIC_CAPITAL_RECOVERY_SELL_PCT,
    TIER_CLASSIC_HOUSE_MONEY_1_MULT, TIER_CLASSIC_HOUSE_MONEY_1_SELL_PCT,
    TIER_CLASSIC_HOUSE_MONEY_2_MULT, TIER_CLASSIC_HOUSE_MONEY_2_SELL_PCT,
    TIER_CLASSIC_LATE_GAME_PRICE_CENTS, TIER_CLASSIC_LATE_GAME_SELL_PCT,
    TIER_CLASSIC_TRAILING_STOP_PCT, TIER_CLASSIC_DEFENSIVE_HARD_FLOOR_PCT,
    TIER_CLASSIC_UNIVERSAL_STOP_PCT, TIER_CLASSIC_RECOVERY_ENTRIES,
...
class TieredClassicStrategy(BaseStrategy):
    """
    Original tiered strategy with fixed multiplier exits.
    Up to 4 entries per game. House money exit system.
    """

    def __init__(self, positions: dict, bankroll_cents: int):
        super().__init__(Strategy.TIERED_CLASSIC, positions)
    ...
    def check_exit(self, state: LiveGameState, position: Position) -> Optional[dict]:
        """Exit logic with fixed multiplier targets + capital recovery mode + universal stop."""
        ...
        # ═══════════════════════════════════════
        # UNIVERSAL STOP LOSS (Q3+ only, -50%)
        ...
        if not position.capital_recovered:
            ...
            if (current_price >= TIER_CLASSIC_LATE_GAME_PRICE_CENTS and
                    state.quarter == 4 and
